...
<div class="container mt-4">
    ...
    <form action="<?= isset($deposito) ? URLROOT . '/deposito/update/' . $deposito->id_deposito : URLROOT . '/deposito/store' ?>" method="POST">
        <div class="mb-3">
        ...
        <div class="mb-3">
            <label for="capacidad_maxima" class="form-label">Capacidad Máxima (opcional, unidades):</label>
            ...
            <small class="form-text text-muted">Cantidad máxima de unidades que este depósito puede contener.</small>
        </div>

        <button type="submit" class="btn btn-success"><?= isset($deposito) ? 'Actualizar Depósito' : 'Crear Depósito' ?></button>
        <a href="<?= URLROOT ?>/deposito/index" class="btn btn-secondary">Cancelar</a>
    </form>
</div>
